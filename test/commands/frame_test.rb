  ...
  class FrameTestCase < TestCase
    ...
    def test_frame_prints_the_callstack_when_called_without_arguments
      ...
      check_output_includes(
        /--> #1  .*encode\(str#String\)\s* at .*#{example_file}:11/)
    end
    ...
    def test_frame_minus_one_sets_frame_to_the_last_one
      enter 'frame -1'
      debug_code(program) { assert_equal example_file, state.file }
    end
    ...
    def test_where_displays_current_backtrace_with_fullpaths
      ...
      debug_code(program)
      check_output_includes(
        /--> #0  .*integerize\(str#String\)\s* at #{example_file}:16/,
            /#1  .*encode\(str#String\)\s* at #{example_file}:11/,
            /#2  .*initialize\(letter#String\)\s* at #{example_file}:7/,
            /ͱ-- #3  Class\.new\(\*args\)\s* at #{example_file}:20/,
            /#4  <module:Byebug>\s* at #{example_file}:20/,
            /#5  <top \(required\)>\s* at #{example_file}:1/)
    end
    ...
    def test_where_displays_current_backtrace_w_shorpaths_if_fullpath_disabled
      ...
      check_output_includes(
        ...
        /#1  .*encode\(str#String\)\s* at .*#{example_file}:11/,
        /#2  .*initialize\(letter#String\)\s* at .*#{example_file}:7/,
        /ͱ-- #3  Class\.new\(\*args\) at .*#{example_file}:20/,
        /#4  <module:Byebug> at .*#{example_file}:20/,
        /#5  <top \(required\)> at .*#{example_file}:1/)
    end
    ...
    def test_where_displays_backtraces_using_short_callstyle
      ...
      debug_code(program)
      check_output_includes(
        /--> #0  integerize\(str\)\s* at #{example_file}:16/,
        /#1  encode\(str\)\s* at #{example_file}:11/,
        /#2  initialize\(letter\)\s* at #{example_file}:7/,
        /ͱ-- #3  new\(\*args\) at #{example_file}:20/,
        /#4  <module:Byebug> at #{example_file}:20/,
        /#5  <top \(required\)> at #{example_file}:1/)
    end
